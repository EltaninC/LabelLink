...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    ExecutorService executorService = new ThreadPoolExecutor
            (10,10,60,TimeUnit.SECONDS,new ArrayBlockingQueue<>(30));
...
//        }
    ...
    public User getLoginUser(HttpServletRequest request) {
        String token = request.getHeader("authorization");
        //调试
        token = "301ad347aca04884b72e4c6e9fa8ee6f";
        if (StrUtil.isBlank(token)) {
            return null;
        }
    ...
    public List<Pair<User, Long>> getArrangedUser(String tags, Long id) {
        long MysqlStime = System.currentTimeMillis();
        ...
        // 计算执行时间
        long MysqlEtime = System.currentTimeMillis();
        System.out.printf("sql执行时长：%d 毫秒.\n", (MysqlEtime - MysqlStime));
        ...
        // 用户列表的下标 => 相似度, 线程安全的数组
        List<Pair<User, Long>> list = Collections.synchronizedList(new ArrayList<>());
        // 依次计算所有用户和当前用户的相似度
        int p = userList.size()/5000;
        CountDownLatch count = new CountDownLatch(p+1);
        for(int i = 0; i <= p; i++){
            List<User> users = userList.subList(i * 5000, Math.min(i * 5000 + 5001, userList.size()));
            // 计算分数
        ...
        try {
            count.await();
            // 按编辑距离由小到大排序
            List<Pair<User, Long>> collect = list.stream()
                    .sorted((a, b) -> (int) (a.getValue() - b.getValue()))
                    .collect(Collectors.toList());

            // 计算执行时间
            long etime = System.currentTimeMillis();
            System.out.printf("算法执行时长：%d 毫秒.\n", (etime - stime));
            return collect;
...
//        userLambdaQueryWrapper.select(User::getId, User::getTags);
//        //线程安全的数组
//        List<User> userList = Collections.synchronizedList(this.list(userLambdaQueryWrapper));
//
//        // 计算执行时间
//        long MysqlEtime = System.currentTimeMillis();
//        System.out.printf("sql执行时长：%d 毫秒.\n", (MysqlEtime - MysqlStime));
        ...
        if (loginUser != null) {
            ...
            if (StringUtils.isNotBlank(username)) { // 填写了用户名,模糊查询
                ...
                if (Boolean.TRUE.equals(hasKey)) { // 存在缓存
                    ...
                    long etime = System.currentTimeMillis();
                    // 计算执行时间
                    System.out.printf("redis执行时长：%d 毫秒.\n", (etime - stime));
